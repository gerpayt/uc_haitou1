...
class usermodel {
	...
	function add_user($username, $realname, $password, $email, $schoolname, $uid = 0, $questionid = '', $answer = '', $regip = '') {
		$regip = empty($regip) ? $this->base->onlineip : $regip;
		$salt = substr(uniqid(rand()), -6);
		$password = md5(md5($password).$salt);
		$sqladd = $uid ? "uid='".intval($uid)."'," : '';
		$sqladd .= $questionid > 0 ? " secques='".$this->quescrypt($questionid, $answer)."'," : " secques='',";
		$this->db->query("INSERT INTO ".UC_DBTABLEPRE."members SET $sqladd username='$username', realname='$realname', password='$password', email='$email', schoolname='$schoolname', regip='$regip', regdate='".$this->base->time."', salt='$salt'");
		$uid = $this->db->insert_id();
		$this->db->query("INSERT INTO ".UC_DBTABLEPRE."memberfields SET uid='$uid'");
		return $uid;
	}

	function edit_user($username, $oldpw, $newpw, $email, $schoolname, $ignoreoldpw = 0, $questionid = '', $answer = '') {
		...
		$sqladd .= $schoolname ? ($sqladd ? ',' : '')." schoolname='$schoolname'" : '';
		if($questionid !== '') {
			if($questionid > 0) {
